...
pub const GALOIS_MULT_2: [u8; 256] = [
...
pub const GALOIS_MULT_3: [u8; 256] = [
...
pub fn mix_columns(state: &mut [[u8; 4]; 4]) {
    ...
    for col in 0..4 {
        ...
        for row in 0..4 {
            state[col][row] = GALOIS_MULT_2[intact_column[row] as usize]
                ^ GALOIS_MULT_3[intact_column[(row + 1) % 4] as usize]
                ^ intact_column[(row + 2) % 4]
                ^ intact_column[(row + 3) % 4];
        }
    }
}
