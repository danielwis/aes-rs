pub const SBOX: [u8; 256] = [
    ,
];
pub fn create_lookup_tables() -> Vec<Vec<u32>> {
    let M = [
        [0x2, 0x1, 0x1, 0x3],
        [0x3, 0x2, 0x1, 0x1],
        [0x1, 0x3, 0x2, 0x1],
        [0x1, 0x1, 0x3, 0x2],
    ];
    let mut T = vec![vec![0u32; 256]; 4]; // 4 tables of 256 u32s each
    for i in 0..4 {
        let mclone = M[i].clone();
        for byte in 0..=255 {
            let mut tmp = [0u8; 4];
            for j in 0..4 {
                 tmp[j] = finite_field_mult(mclone[j], SBOX[byte]);
            }
            T[i][byte] = u32::from_be_bytes(tmp);
        }
    }

    T
}

// Thanks to https://en.wikipedia.org/wiki/Finite_field_arithmetic#Rijndael's_(AES)_finite_field
// Bit shifting has to be &-ed as otherwise it expands (e.g. to an u16 or u32)
pub fn finite_field_mult(mut a: u8, mut b: u8) -> u8 {
    let mut p: u8 = 0;

    for _ in 0..8 {
        // 0*0 is just 0. However, this makes a timing attack possible.
        if a == 0 || b == 0 {
            return p;
        }

        if b & 1 == 1 {
            p ^= a;
        }

        b = (b >> 1) & 0xff;

        let carry: bool = (a & 0x80) == 0x80;

        a = (a << 1) & 0xff;

        if carry {
            a ^= 0x1b;
        }
    }

    return p;
}

fn main() {
    let v = create_lookup_tables();
    dbg!(v);
}
